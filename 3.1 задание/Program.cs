...
namespace program
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Combinations<char> letter = new Combinations<char>(alf_char, 1); // Выбираем букву

            do {
                Combinations<int> C = new Combinations<int>(alf_ind, 2); // Выбираем 2 индекса, на которые будет поставлена выбранная буква
                do
                {
                    List<int> ForA = C.Obj; // Получаем лист индексов, на которые будут поставлены выбранные буквы
                    List<int> ForB = Sets<int>.FindSubtract(param, C.Obj); // Находим индексы, на которые НЕ будет поставлена выбранная буква
                    Alphabet<char> not_letter = new Alphabet<char>(Sets<char>.FindSubtract(letters, letter.GetNode)); // Находим все буквы, кроме выбранной
                    Placement<char> P = new Placement<char>(not_letter, 3); // Выбираем 3 символа из алфавита
                    do
                    {
                        ...
                        foreach (int i in ForA) // Расставляем выбранную букву на выбранные позиции
